...
const inlines = {
  ...
  css: fs.readFileSync(
    path.join(viewPath, "inlines", "bootstrap.css"),
    "utf-8"
  ),
...
const defaultViewOptions = {
  ...
  shows: library.shows,
  version,
  scripts: ["dist/client/scripts/app.js"],
};
...
dynamic.get("/*", (req, res) => {
  ...
  const watchMore = videoLibrary.getOtherTitlesInShow(
    library.shows,
    pathParts[0],
    fullPath
  );
  ...
  const viewOptions = Object.assign({}, defaultViewOptions, {
    ...
    item: search.items,
    css: ["dist/client/styles/biograf.css"],
    inlines: {
      js: inlines.js,
    },
    fullPath,
